#!/usr/bin/env python3
"""
FastVLM multi-pass tester with 5 prompts:
- Pass 1: person description (gender, age, build, hair)
- Pass 2: torso clothing (all upper body clothing items)
- Pass 3: leg clothing (pants, skirts, shorts, etc.)
- Pass 4: shoes and footwear
- Pass 5: accessories (jewelry, bags, belts, etc.)
...
def main():
    ...
    # Pass 2: all torso clothing (outerwear + inner clothing)
    r2 = post_analyze(TORSO_CLOTHING_PROMPT, b64_full)
    t2 = time.perf_counter()
    torso_clothing = extract_text(r2)

    # Pass 3: leg clothing (pants, skirts, shorts)
    r3 = post_analyze(LEG_CLOTHING_PROMPT, b64_full)
    t3 = time.perf_counter()
    leg_clothing = extract_text(r3)

    # Pass 4: shoes
    r4 = post_analyze(SHOES_PROMPT, b64_full)
    t4 = time.perf_counter()
    shoes = extract_text(r4)

    # Pass 5: accessories
    r5 = post_analyze(ACCESSORIES_PROMPT, b64_full)
    t5 = time.perf_counter()
    accessories = extract_text(r5)
    ...
    report = {
        ...
        "timings_seconds": {
            ...
            "torso_clothing": round(t2 - t1, 3),
            "leg_clothing": round(t3 - t2, 3),
            "shoes": round(t4 - t3, 3),
            "accessories": round(t5 - t4, 3),
            "total": round(t5 - t0, 3)
        }
    }
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("PERSON\n" + person.split("\n")[0].strip() + "\n\n")
        f.write("TORSO CLOTHING\n" + (torso_clothing or "none") + "\n\n")
